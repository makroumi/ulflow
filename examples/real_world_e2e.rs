...
fn bench<F: FnMut()>(name: &str, iters: u32, mut f: F) -> f64 {
    ...
    if ns >= 1000.0 { println!("    {:<45} {:>10.1} us", name, ns/1000.0); }
    else { println!("    {:<45} {:>10.0} ns", name, ns); }
    ns
}
...
fn main() {
    ...
    { let mut eng = engine.write().unwrap();
      for (k, v) in &files { eng.put(k.as_bytes(), v.as_bytes()).unwrap(); } }
    ...
    registry.register_tool(
        ToolDef::new("code_search", "Search").param("query", "Q", ParamType::String, true).tag("search"),
        Box::new(move |call| {
            ...
            let hits = eng.indices.query(&uldb::query::planner::QuerySpec {
                text: q.to_string(), top_k: 5, ..Default::default() });
            let r: Vec<String> = hits.iter().map(|h| String::from_utf8_lossy(&h.key).to_string()).collect();
            ToolResult { call_id: call.call_id.clone(), status: ToolStatus::Success,
                output: ToolValue::String(r.join("\n")), error: None,
                tokens_used: Some(r.len() * 20), latency_ms: None }
        }),
    );

    let eng_r: Arc<RwLock<Engine>> = Arc::clone(&engine);
    registry.register_tool(
        ToolDef::new("file_read", "Read").param("path", "P", ParamType::String, true).tag("io"),
        Box::new(move |call| {
            ...
            match eng.get(p.as_bytes()) {
                Some(d) => {
                    ...
                    let len = d.len();
                    ToolResult { call_id: call.call_id.clone(), status: ToolStatus::Success,
                        output: ToolValue::String(text), error: None,
                        tokens_used: Some(len / 4), latency_ms: None }
                }
                None => ToolResult { call_id: call.call_id.clone(), status: ToolStatus::Error,
                    output: ToolValue::Null, error: Some(format!("not found: {}", p)),
    ...
    registry.register_tool(
        ToolDef::new("file_write", "Write")
            ...
            .param("content", "C", ParamType::String, true).tag("io"),
        Box::new(move |call| {
            let p = call.arguments.get("path").and_then(|v| v.as_str()).unwrap_or("");
            ...
            match eng.put(p.as_bytes(), c.as_bytes()) {
                ...
                Err(e) => ToolResult { call_id: call.call_id.clone(), status: ToolStatus::Error,
                    output: ToolValue::Null, error: Some(e.to_string()),
                    tokens_used: None, latency_ms: None },
            }
        }),
    );
    ...
    runner.on_event(move |e| { ec.lock().unwrap().push(e.kind.to_string()); });

    let result = runner.run(flow, FlowInput::new().var("task", "Cache RSA key")).unwrap();
    check!("workflow succeeded", result.succeeded());
    check!("4 steps done", result.steps_completed == 4);
    println!("    Status: {}, Steps: {}, Tokens: {}, Latency: {}ms",
    ...
    } else { check!("search found code", false); }
    ...
    println!("\n--- 4. ULMEN-CORE: Serialize ---");
    let records = vec![
        AgentRecord { record_type: RecordType::Msg, id: "m1".into(), thread_id: "s42".into(), step: 1,
            fields: vec![FieldValue::Str("user".into()), FieldValue::Int(1),
                FieldValue::Str("Refactor auth".into()), FieldValue::Int(5), FieldValue::Bool(false)],
            meta: MetaFields::default() },
        AgentRecord { record_type: RecordType::Tool, id: "t1".into(), thread_id: "s42".into(), step: 2,
            fields: vec![FieldValue::Str("search".into()), FieldValue::Str("{}".into()),
                FieldValue::Str("done".into())],
            meta: MetaFields::default() },
        AgentRecord { record_type: RecordType::Res, id: "t1".into(), thread_id: "s42".into(), step: 3,
            fields: vec![FieldValue::Str("search".into()), FieldValue::Str("auth.py".into()),
                FieldValue::Str("done".into()), FieldValue::Int(12)],
            meta: MetaFields::default() },
    ];
    let payload = AgentPayload { header: AgentHeader { thread_id: Some("s42".into()),
        record_count: 3, ..Default::default() }, records };
    ...
    let comp = compress_context(&dec.records, CompressStrategy::CompletedSequences, 2, None, None, false);
    check!("compress", comp.len() < 3);
    println!("    {} bytes, {} -> {} records", encoded.len(), 3, comp.len());
    ...
    } else { check!("survives restart", false); }

    // === 6. Memory + Checkpoint ===
    println!("\n--- 6. Memory + Checkpoint ---");
    let mut mem = Memory::new();
    mem.store("fix", "cached key", MemoryScope::Session, 0.95);
    check!("memory", mem.get_value("fix").and_then(|v| v.as_str()) == Some("cached key"));
    ...
    check!("checkpoint", cp2.is_step_completed("search") && cp2.tokens_used == 200);

    // === 7. Benchmarks ===
    println!("\n--- 7. Benchmarks ---");
    bench("ulmen-core encode 3 records", 10_000, || { let _ = payload.encode(); });
    bench("ulmen-core decode 3 records", 10_000, || { let _ = AgentPayload::decode(&encoded); });
    bench("ulmen-core validate", 10_000, || { let _ = validate_payload(&payload); });
    bench("context tracker cycle", 100_000, || {
        let mut c = ContextTracker::new(4096); c.use_tokens(100); c.reserve(200); let _ = c.available(); });
    bench("memory store+get", 100_000, || {
        let mut m = Memory::new(); m.store("k","v",MemoryScope::Session,0.9); let _ = m.get_value("k"); });
    bench("checkpoint save+load", 10_000, || {
        let b = cp.to_bytes(); let _ = Checkpoint::from_bytes(&b); });
